...
<a href="http://ossrndc.in" target="_blank"><div class="powered-by-team-oss black-text"> <span class="green-text text-darken-2">GreenBoard</span>, Powered by Team OSS</div></a>
...
if(!isset($_SESSION['ses_name']) && isset($_POST['rollno'])){
  ...
  $branch = strtoupper(substr($row['section'],0,strlen($row['section'])-2));
     echo '
     <div class="row">
            ...
            <div id="theory'.$modal_no_theory.'" class="modal bottom-sheet prmodal">
              ...
              <div class="container">
                ...
                <div class="row fxd grey lighten-5">
                  ...
                  <div class="col s6 m6">
                  <div class="col s12 m12">
                    <b>Name:</b><div class="right">'.ucwords($row["name"]).'</div>
                  </div>
                  <div class="col s12 m12">
                    <b>Department</b>:<div class="right">'.strtoupper($fac["department"]).'</div>
                  </div>
                  <div class="col s12 m12">
                    <b>Name of faculty:</b><div class="right">'.ucwords($fac["name"]).'</div>
                  </div>
                  <div class="col s12 m12">
                    <b>Subject Code:</b><div class="right">'.strtoupper($row[$th]).'</div>
                  </div>
                  </div>

                  <div class="col s6 m6">
                  <div class="col s12 m12">
                  <b>Subject Name:</b><div class="right">'.strtoupper($row[$th1]).'</div>
                  </div>
                  <div class="col s12 m12">
                  <b>Branch/Year:</b><div class="right">'.$branch.'/'.$row["year"].'</div>
                  </div>
                  <div class="col s12 m12">
                  <b>Section:</b><div class="right">'.strtoupper($row["section"]).'</div>
                  </div>
                  <div class="col s12 m12">
                  <b>Date:</b><div class="right">'.$date.'</div>
                  </div>
                  </div>
</div>
                </div>
          ...
          <div id="practical'.$modal_no_practical.'" class="modal bottom-sheet prmodal">
            ...
            <div class="container">
              ...
              <div class="row fxd grey lighten-5">
                ...
                <div class="col s12 m12">
                  <b>Department:</b><div class="right">'.strtoupper($fac_1["department"]).'</div>
                </div>
                <div class="col s12 m12">
                  <b>Subject Code:</b><div class="right">'.strtoupper($row[$code]).'</div>
                </div>
                <div class="col s12 m12">
                <b>Date:</b><div class="right">'.$date.'</div>
                </div>
                ...
                <div class="col s6 m6">
                <div class="col s12 m12">
                <b>Subject Name:</b><div class="right">'.strtoupper($row[$code1]).'</div>
                </div>
                <div class="col s12 m12">
                <b>Branch/Year:</b><div class="right">'.$branch.'/'.$row["year"].'</div>
                </div>
                ...
                <div class="col s12 m12">
                  <b><br>Name of faculties:</b><div style="margin-right:50%;!important" class="right">'.ucwords($fac_1["name"]);
                  if($fac_2["name"]!='')
                  echo' & '.ucwords($fac_2["name"]).'</div>
                </div>';
                else
                  echo '</div></div>';
                ...
                echo'
                <div class="col s12 m12">
                  <b>Name of lab assistant:</b><div style="margin-right:50%;!important" class="right">'.ucwords($la["name"]).'</div>
                </div>';
                $la_name[0]=ucwords($la["name"]);
                }
 ...
 while($disp_ques  = mysqli_fetch_assoc($qu)){
  ...
  echo ' <div class="row">
    ...
    <div class="col s3 m3">'.ucwords($la_name[0]).'</div>
    </div>';
  $number=1;
  if($modal_no_practical!=3){
 while($disp_ques2  = mysqli_fetch_assoc($qu2)){
  ...
  for($o=1;$o<$m;$o++){
    ...
    <div class="col s6 m6"></div>
    <div class="col s3 m3"></div>
    <div class="col s3 m3">'.ucwords($la_name[$o]).'</div>
    </div>';
   $number=1;
  while($disp_ques2  = mysqli_fetch_assoc($qu2)){
  ...
  foreach ($subject['name'] as $key => $value) {
    ...
    echo'<div class="col s2 m2"><a class="waves-effect waves-light btn green darken-2 modal-trigger" href="#theory'.$j.'" id="theory-button-'.$j.'">'.$value.'</a></div>';
  }else{
    echo'<div class="col s2 m2"><a class="waves-effect waves-light btn btn green darken-2 modal-trigger disabled" href="#theory'.$j.'" id="theory-button-'.$j.'">'.$value.'</a></div>';
  }
    $j++;
  }
  echo '</div>';
...
foreach ($practical_sub['name'] as $key => $value) {
  if(!$practical_sub['disabled'][$j]){
    echo'<div class="col s2 m2"><a class="waves-effect waves-light btn green darken-2 modal-trigger" href="#practical'.$j.'" id="practical-button-'.$j.'">'.$value.'</a></div>';
  }else{
    echo'<div class="col s2 m2"><a class="waves-effect waves-light btn green darken-2 modal-button disabled" href="#practical'.$j.'" id="practical-button-'.$j.'">'.$value.'</a></div>';
  }
$j++;
}
echo '</div>';
